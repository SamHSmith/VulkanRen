    ...
    new VkDebugReportCallbackEXTI() {
		public int invoke(int flags, int objectType, long object, long location, int messageCode, long pLayerPrefix,
				long pMessage, long pUserData) {
				    ...
				    System.err.format(
				        "%s: [%s] Code %d : %s\n",
				        type, MemoryUtil.memASCII(pLayerPrefix), messageCode, VkDebugReportCallbackEXT.getString(pMessage)
				    );
...
private void demo_set_image_layout(long image, final int aspectMask, int old_image_layout, int new_image_layout, int srcAccessMask) {
    try (MemoryStack stack = MemoryStack.stackPush()) {
        if (setup_cmd == null) {
        ...
        switch (new_image_layout) {
            ...
            case VK10.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL:
                ...
                image_memory_barrier.dstAccessMask(VK10.VK_ACCESS_SHADER_READ_BIT | VK10.VK_ACCESS_INPUT_ATTACHMENT_READ_BIT);
                break;
...
private void demo_prepare_depth() {
    ...
    try (MemoryStack stack = MemoryStack.stackPush()) {
        ...
        demo_set_image_layout(depth.image, VK10.VK_IMAGE_ASPECT_DEPTH_BIT,
...
private void demo_prepare_textures() {
    ...
    try (MemoryStack stack = MemoryStack.stackPush()) {
        ...
        for (int i = 0; i < DEMO_TEXTURE_COUNT; i++) {
            if ((props.linearTilingFeatures() & VK10.VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) != 0 && !USE_STAGING_BUFFER) {
...
private void demo_prepare_pipeline() {
    ...
    try (MemoryStack stack = MemoryStack.stackPush()) {
        ...
        VkPipelineDepthStencilStateCreateInfo ds = VkPipelineDepthStencilStateCreateInfo.callocStack(stack)
            ...
            .back(new Consumer<VkStencilOpState>() {
				@Override
...
private void demo_draw_build_cmd() {
    try (MemoryStack stack = MemoryStack.stackPush()) {
        ...
        VkRenderPassBeginInfo rp_begin = VkRenderPassBeginInfo.mallocStack(stack)
            ...
            .renderArea(new Consumer<VkRect2D>() {
				@Override
...
private void demo_draw() {
    try (MemoryStack stack = MemoryStack.stackPush()) {
        ...
        int err = KHRSwapchain.vkAcquireNextImageKHR(device, swapchain, ~0L,
            imageAcquiredSemaphore,
            0, // TODO: Show use of fence
            ip);
